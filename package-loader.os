...
Процедура ПриЗагрузкеБиблиотеки(Путь, СтандартнаяОбработка, Отказ)
	...
	Если Осень <> Неопределено Тогда
		...
		// FIXME: Выработать решение с порядком инициализации заготовок после вернуть закоментированный код
		// взамен подключения сценарию, на текущий момент заготовка инициализируется до инициализации логоса
		//	ОбработатьКаталогКлассов(ОбъединитьПути(Путь, "src/Заготовки"), СтандартнаяОбработка, Отказ);
		ПодключитьСценарий(ОбъединитьПути(Путь, "src", "Заготовки", "ИнициализацияАсинх.os"), "ИнициализацияАсинх");
